...
def getOptions():
    ...
    parser.add_option("--E", dest = "serviceAccountEmail", help = "service account email address", 
                      metavar = "STRING", 
                      default = "875996339847-nv3l8p9pp4ervtpsg1gbpbabktd619db@developer.gserviceaccount.com",#"994571973726-80h6vd6u4u7s3u8s28ekdqdcng178cbc@developer.gserviceaccount.com", 
                      type = "string")
    ...
    parser.add_option("--PM", dest = "pemFile", help = "path to pem file", 
                      metavar = "FILE", default = "./GCP_SnyderProject.pem",#"./4e3bba34a0aef1b5ba6178a5c06de168fa266515-privatekey.pem", 
                      type = "string")
...
class JobExecutionLoop(object):
    ...
    def __init__(self, log_file, job_csv_file, disk_csv_file, service_account_email_address, 
                 project_id, pem_file, data_center, auth_type, metadata_url, storage_directory, cycle_period=60, max_instances=23, restart=False, 
                 commandFile = ""):
        '''
        Constructor
        '''
    ...
    def run(self):
        ...
        self.jobManager=JobManager(self.job_csv_file, self.disk_csv_file, self.myDriver, self.log, self.storage_directory, self.max_instances, update= (not self.restart))
...
if __name__ == '__main__':
    ...
    engine=JobExecutionLoop(options.logFilePath, options.InstancesFile, options.DisksFile,
                            ...
                            options.storageDirectory, max_instances=int(options.maxInstances),
